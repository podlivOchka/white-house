...
for(const [route,title] of pages){
 ...
 const head='<meta property="og:title" content="'+escape(title)+'"/><meta property="og:description" content="'+escape(description)+'"/><meta property="og:type" content="website"/><meta property="og:locale" content="ru_RU"/>'+
   (preview||route==="/favorites"?'<meta name="robots" content="noindex, nofollow"/>':"")+
   (siteUrl&&!preview?'<link rel="canonical" href="'+escape(absoluteUrl(route))+'"/>':"")+
   '<script type="application/ld+json">'+JSON.stringify(schema).replace(/</g,"\\u003c")+'</script>';
 const fallback='<div id="root"><main><h1>'+escape(title)+'</h1><p>'+escape(description)+'</p><p>'+escape(store.address)+'</p><nav><a href="'+escape(localUrl("/catalog"))+'">Каталог одежды</a></nav><ul>'+products.map(p=>'<li><a href="'+escape(localUrl("/product/"+p.id))+'">'+escape(p.name)+'</a></li>').join("")+'</ul></main></div>';
...
if(siteUrl&&!preview)await writeFile("dist/sitemap.xml",'<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'+pages.filter(p=>p[0]!=="/favorites").map(p=>"<url><loc>"+escape(absoluteUrl(p[0]))+"</loc></url>").join("")+"</urlset>");
