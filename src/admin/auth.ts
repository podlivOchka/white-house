...
export type Session={provider:"server"|"netlify";user:{email:string}|null};
...
export async function signOut(provider:Session["provider"]){
  if(provider==="netlify")await (await identity()).logout();else await request("/api/auth/logout",{method:"POST"});
}
