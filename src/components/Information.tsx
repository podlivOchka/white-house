...
if(info==="store")return <div className="info-content"><p><MapPin size={19}/>{store.address}</p><p>{store.hours}</p><a className="info-phone" href={store.telephone}>{store.phone}</a><a className="dark-button full-width" href={store.maps} target="_blank" rel="noreferrer">Открыть Яндекс Карты<ArrowUpRight size={17}/></a><SocialProfiles compact/></div>;
