import {getProduct,money,products,sizes} from "../data/catalog.ts";
...
for(const item of value.slice(0,100)){
 ...
 if(!p||!sizes.includes(item.size)||typeof item.color!=="string"||(!p.colors.includes(item.color)&&item.color!=="Уточнить цвет")||!Number.isInteger(item.quantity)||item.quantity<1||item.quantity>99)continue;
...
export function parseFavorites(value:unknown):string[]{return Array.isArray(value)?[...new Set(value.filter((id):id is string=>typeof id==="string"&&products.some(p=>p.id===id)))]:[];}
export function addLine(cart:CartLine[],line:CartLine):CartLine[]{return parseCart([...cart,line]);}
export function total(cart:CartLine[]){return {amount:cart.reduce((n,l)=>n+(getProduct(l.id)?.price??0)*l.quantity,0),unknown:cart.some(l=>getProduct(l.id)?.price===null),count:cart.reduce((n,l)=>n+l.quantity,0)};}
